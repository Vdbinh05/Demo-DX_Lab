    ...
    <AdminModal title={`Chi tiết đơn ${orderId}`} subtitle="Dữ liệu được truy xuất trực tiếp từ lịch sử bán hàng" onClose={onClose} wide>
      {loading ? <LoadingState /> : error ? <ErrorState message={error} onRetry={reload} /> : (
        <div className="order-detail">
          ...
          <div className="order-detail-total"><div><span>Tạm tính</span><b>{money(data.subtotal_value ?? data.total_value)}</b></div>{Number(data.discount_value) > 0 && <div><span>Khuyến mãi {data.promotion_title ? `· ${data.promotion_title}` : ""}</span><b>− {money(data.discount_value)}</b></div>}<div><span>Tổng thanh toán</span><strong>{money(data.total_value)}</strong></div></div>
        </div>
      )}
    </AdminModal>
  );
  ...
  const submit = async (event) => {
    ...
    try {
      const body = {
        ...
        ...(!item ? { product_id: form.product_id, stock: Number(form.stock) } : {}),
      };
      await apiRequest(item ? `/admin/products/${encodeURIComponent(item.product_id)}` : "/admin/products", { token, method: item ? "PUT" : "POST", body });
      onSaved(item ? "Đã cập nhật sản phẩm." : "Đã thêm sản phẩm mới.");
    } catch (requestError) { setError(requestError.message); } finally { setSaving(false); }
  };
  return <AdminModal title={item ? "Cập nhật sản phẩm" : "Thêm sản phẩm"} subtitle="Thay đổi được lưu trực tiếp vào SQL Server" onClose={onClose}><form className="admin-form" onSubmit={submit}>{error && <div className="form-error">{error}</div>}<label>Mã sản phẩm<input value={form.product_id} onChange={(event) => set("product_id", event.target.value)} disabled={Boolean(item)} placeholder="Để trống để hệ thống tự tạo" /></label><label>Tên sản phẩm<input required minLength="2" value={form.name} onChange={(event) => set("name", event.target.value)} /></label><div className="form-row"><label>Danh mục<input required value={form.category} onChange={(event) => set("category", event.target.value)} /></label><label>Giá bán<input required type="number" min="0" value={form.price} onChange={(event) => set("price", event.target.value)} /></label></div><div className="form-row">{item ? <label>Tồn kho<input value={form.stock} disabled /><small>Điều chỉnh số lượng tại trang Kho hàng để hệ thống lưu lịch sử.</small></label> : <label>Tồn kho ban đầu<input required type="number" min="0" value={form.stock} onChange={(event) => set("stock", event.target.value)} /></label>}<label>Mức cảnh báo<input required type="number" min="0" value={form.reorder_level} onChange={(event) => set("reorder_level", event.target.value)} /></label></div><div className="form-actions"><button type="button" onClick={onClose}>Hủy</button><button className="admin-primary" disabled={saving}>{saving && <LoaderCircle className="spin" size={16} />}{item ? "Lưu thay đổi" : "Thêm sản phẩm"}</button></div></form></AdminModal>;
}
